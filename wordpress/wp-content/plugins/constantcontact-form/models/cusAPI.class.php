 ...
 * @api
 * @version 0.4
 * @since 2.0 First time this was introduced into Constant Contact Signup Form plugin.
...
class cUsComAPI_CtCt {
     ...
     * @api
     */
    var $enviroment = cUs_CtCt_API_ENV;
    var $wpApiAccountKey = cUs_CtCt_API_ACC;
    var $wpApiKey = cUs_CtCt_API_AKY;
    ...
    public function getAPICredentials($cUs_email, $cUs_pass) {
        ...
        if( $this->_isCurl() ){
            ...
            curl_setopt($ch, CURLOPT_HTTPHEADER, array('X-ContactUs-Request-URL: ' . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'], 'X-ContactUs-Signature: ctct|3.0|' . $this->getIP()));
    ...
    public function createCustomer($postData, $credential) {
        ...
        if (!strlen($postData['fname'])) {
            ...
            if( $this->_isCurl() ){
                ...
                $strCURLOPT .= '&Promotion_Code=WPCC';
                $strCURLOPT .= '&Version=ctct|3.0';
                ...
                curl_setopt($ch, CURLOPT_HTTPHEADER, array(
                    ...
                    'X-ContactUs-Signature: ctct|3.0|' . $this->getIP()
                ));
    ...
    public function getDeliveryOptions($cUs_API_Account, $cUs_API_Key, $formkey) {
        ...
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_USERAGENT, $_SERVER['HTTP_USER_AGENT']);
        curl_setopt($ch, CURLOPT_HTTPHEADER, array('X-ContactUs-Request-URL: ' . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'], 'X-ContactUs-Signature: mc|2.3.2|' . $this->getIP(),));
        $content = curl_exec($ch);
        curl_close($ch);
    ...
    public function getDefaultDeepLink($cUs_CtCt_API_getFormKeys) {

        if (empty($cUs_CtCt_API_getFormKeys))
            return FALSE;

        //$cUs_CtCt_API_getKeysResult = $this->getFormKeysData($cUs_API_Account, $cUs_API_Key);
        $cUs_jsonKeys = json_decode($cUs_CtCt_API_getFormKeys);
    ...
    public function resetData() {
        ...
        //PLUGIN OPTIONS
        delete_option('cUs_CtCt_settings');
        delete_option('cUs_CtCt_settings_userData');
        delete_option('cUs_CtCt_settings_FORM');
        delete_option('cUs_CtCt_HOME_settings');
        delete_option('cUs_CtCt_settings_step1');
        ...
        delete_option('cUs_CtCt_settings_userCredentials');
        delete_option('cUs_CtCt_settings_default_deep_link_view');
        delete_option('cUs_CtCt_settings_form_id');
        delete_option('cUs_CtCt_settings_intro_hints');

        cUs_CtCt_page_settings_cleaner(); //REMOVE CUSTOM SETTINGS BY PAGE
        cUs_CtCt_inline_shortcode_cleaner(); //REMOVE SHORTCODES
