...
cUs_CtCt_myjq(document).ready(function($) {
    ...
    $('.introjs-skipbutton').on('click',function(){
        ...
        bootbox.confirm("User guide will appear again using the help button above.", function(result) {
            if(result){
                $.ajax({ type: "POST", url: ajax_object.ajax_url, data: {action:'cUs_CtCt_disable_introjs'},
...
function startIntro(){
    ...
    intro.setOptions({
        steps: [
            ...
            {
                //7
                element: '.mc_navigate_sync',
                intro: "<div class='intro_box_ai intro'><h1>Automatic Synching with your Constant Contact account</h1><ul><li>The form captures the lead and sends it to specific Constant Contact lists automatically.</li><li>Also add CAPCHA to avoid spam.</li></ul></div>",
                position: "bottom"
            },
            {
                //8
