...
if (class_exists('WC_Payment_Gateway')) {
	...
	if ( ! class_exists( 'WC_Gateway_Inicis_Kpay' ) ) {
    
		class WC_Gateway_Inicis_Kpay extends WC_Gateway_Inicis {
			public function __construct() {
				parent::__construct();
				global $inicis_payment;
				
				$this->id = 'inicis_kpay';
				$this->method_title = __('KPAY(간편결제)', 'inicis_payment');
				...
				$this->icon = $inicis_payment->plugin_url() . '/assets/images/kpay_logo.jpg';

				$this->init_settings();
	            $this->settings['gopaymethod'] = 'onlykpay';
				...
				if( empty($this->settings['title']) ){
					...
					$this->description = __('KPAY는 모바일 및 크롬에서 결제가 가능하나 맥환경에서는 사용할 수 없습니다.', 'inicis_payment');
				}else{
            ...
            function thankyou_page( $order_id ) {
                echo __('KPAY(간편결제) 결제로 결제되었습니다. 감사합니다.', 'inicis_payment');
            }
		...
		if ( defined('DOING_AJAX') ) {
			$ajax_requests = array('payment_form_inicis_kpay', 'refund_request_inicis_kpay');
			if( in_array( $_REQUEST['action'], $ajax_requests ) ){
				new WC_Gateway_Inicis_Kpay();
			}
		}	
	}
