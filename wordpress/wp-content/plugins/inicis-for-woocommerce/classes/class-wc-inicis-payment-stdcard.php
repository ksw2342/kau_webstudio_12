	...
	if ( ! class_exists( 'WC_Gateway_Inicis_Stdcard' ) ) {
		...
		class WC_Gateway_Inicis_Stdcard extends WC_Gateway_Inicis {
			public function __construct() {
				...
				$this->method_title = __('PC 신용카드(웹표준)', 'inicis_payment');
				...
				if( empty($this->settings['title']) ){
					$this->title =  __('신용카드 결제', 'inicis_payment');
					$this->description = __('구글크롬, IE, Safari 에서 결제 가능한 웹표준 결제 입니다 결제를 진행해 주세요.', 'inicis_payment');
				}else{
			...
			function init_form_fields() {
				...
				$this->form_fields = array_merge($this->form_fields, array(
					'quotabase' => array(
						...
						'type' => 'text', 
						'description' => __('할부 구매를 허용할 개월수를 설정하세요.<span style="color:red;">(무이자 할부 개월수가 아닙니다)</span><br/>예) 3:6:9:12 로 입력시 3,6,9,12개월 할부 허용<br/>단, 최소 결제금액이 5만원 이상인 경우에만 할부 결제가 허용됩니다. 지정한 할부 개월수와 상관없이 할부 결제 최소 금액이 아닌 경우 할부 거래가 허용되지 않습니다.', 'inicis_payment'),
						'default' => __('3:6:9:12', 'inicis_payment'),
						'desc_tip' => true, 
						), 
					'nointerest' => array(
						...
						'default' => '',
						'description' => __('wordpressshop.co.kr 사이트의 친절한 메뉴얼을 참고하여 입력하시기 바랍니다. 무이자 할부는 이니시스 계약시 무이자 할부 계약이 체결되어 있어야 합니다. 단, 최소 결제금액이 5만원 이상인 경우에만 할부 결제가 허용됩니다. 무이자 할부 허용하시면 무이자 할부에 따른 수수료는 상점에서 부담하게 됩니다. 수수료는 이니시스에 문의하여 주십시오. (단, 이니시스에서 모든 가맹점을 대상으로 하는 무이자 이벤트인 경우는 제외입니다)', 'inicis_payment'),
						'desc_tip' => true, 
						), 
                    'cardpoint' => array(
		...
		if ( defined('DOING_AJAX') ) {
			$ajax_requests = array('payment_form_inicis_stdcard', 'refund_request_inicis_stdcard');
			if( in_array( $_REQUEST['action'], $ajax_requests ) ){
				new WC_Gateway_Inicis_Stdcard();
			}
		}	
	}
