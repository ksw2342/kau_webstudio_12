...
if( class_exists('WC_Payment_Gateway') ) { 
    ...
    class WC_Gateway_Inicis extends WC_Payment_Gateway{
        public function __construct(){
            ...
            add_filter( 'woocommerce_available_payment_gateways', array($this, 'inicis_gateway_check_process'), 99, 1 );
        }

        function inicis_gateway_check_process($_available_gateways){
            //모바일일 경우 웹표준 결제만 노출
            if( wp_is_mobile() ) {
                unset($_available_gateways['inicis_stdcard']);
            }

            //PC일때, 웹표준이 있으면 기존 신용카드 결제는 숨김 처리
            if(isset($_available_gateways['inicis_stdcard'])) {
                unset($_available_gateways['inicis_card']);
            }

            //크롬 브라우저 & 윈도우OS인 경우 웹표준 결제 제외한 내용은 모두 숨김 처리
            if(strpos($_SERVER['HTTP_USER_AGENT'], 'Chrome') !== FALSE && strpos($_SERVER['HTTP_USER_AGENT'], 'Windows') !== FALSE) {
                unset($_available_gateways['inicis_card']);
                unset($_available_gateways['inicis_bank']);
                unset($_available_gateways['inicis_escrow_bank']);
                unset($_available_gateways['inicis_hpp']);
                unset($_available_gateways['inicis_kpay']);
                unset($_available_gateways['inicis_vbank']);
            }
            ...
            if(isset($_REQUEST['txnid']))
            {
                ...
                try
                {
                    ...
                    if(strtolower($paymethod) == 'vbank') {
                        ...
                        $order->update_status($this->settings['order_status_after_payemnt']);
                    } else {
                        wc_add_notice( __( '결제가 정상적으로 완료되었습니다.', 'inicis_payment'), 'success' );
            ...
            if(!empty($this->id)){
                switch($this->id){
                    ...
                    case 'inicis_stdcard':  //웹표준
                        ...
                        $cardQuotaBase = $this->settings['quotabase']; //가맹점에서 사용할 할부 개월수 설정
            ...
            if (!empty($_REQUEST)) {
                ...
                if (!empty($_REQUEST['type'])) {
                    ...
                    if( $_REQUEST['txnid'] ) {
                        ...
                        $orderid = explode('_', $_REQUEST['orderNumber']);
        ...
        function inicis_print_log($msg, $type='ININoti')
        {
            ...
            $file = $type . $this->merchant_id ."_".date("Ymd").".log";
        ...
        function get_accpetmethod(){
            ...
            if ($this->id == 'inicis_card' && !empty($this->settings)) {
                ...
                $acceptmethod = implode(":", $arr_accept_method);
            } else if ($this->id == 'inicis_kpay' && !empty($this->settings)) {
                if (wp_is_mobile()) {
                    $arr_accept_method[] = 'd_kpay=Y';
                    $arr_accept_method[] = 'kpay_siteId=KPAY';

                    if ($this->settings['direct_run'] == 'yes') {
