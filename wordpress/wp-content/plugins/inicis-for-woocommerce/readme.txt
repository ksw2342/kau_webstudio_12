...
* 워드프레스 4.3 또는 최신 버전 (Wordpress 4.3 or later)
* 우커머스 2.4 또는 최신 버전 (WooCommerce 2.4 or later)
* PHP 5.3.0 또는 최신 버전 (PHP 5.3.0 or later)
* PHP 확장(Extension): OpenSSL, LibXML, mcrypt, mbstring, socket 설치필요 (--with-openssl, --with-mcrypt, --enable-sockets)
...
= 4.2.0 - 2015/11/19 =
* 이니시스 웹표준 카드결제 결제수단 추가
* 이니시스 웹표준 카드결제 우선적용 처리
  - 기존 이니시스 카드와 웹표준 이니시스 카드가 모두 활성화된 경우 신규 이니시스 카드만 노출
* IE 브라우저에서 결제 진행시 상단 공백 노출 현상 추가 보완
* 결제 라이브러리 업데이트 처리 변경
* 결제 라이브러리 업데이트시 쓰기 권한 없는 경우 안내처리 추가
* 결제 취소시 에러 발생하는 경우 에러 내용 노출 처리

= 4.1.4 - 2015/10/21 =
* 가상계좌 무통장입금 - 주문접수시 변경될 주문상태 옵션 동작 수정

= 4.1.3 - 2015/10/01 =
* SSL 사용시 P_NOTI_URL 수신 불가 현상 해결

= 4.1.2 - 2015/09/24 =
* 이니시스 결제 수단 활성화 여부에 따라 스크립트 로딩 처리 변경

= 4.1.1 - 2015/09/15 =
* IE 브라우저에서 결제 진행시 상단 공백 노출 현상 수정
